...
class ToDoTableViewController: UITableViewController, addEditViewControllerDelegate {
  ...
  lazy var toDoListIsEmpty = toDoList[0].toDos.isEmpty && toDoList[1].toDos.isEmpty && toDoList[2].toDos.isEmpty
  ...
  var toDoList: [Category] = [
        Category(group: .high,
          toDos: [ToDo(title: "House chore", todoDescription: "Wash the dishes", priority: .high, isCompleted: false)
                 ]),
        Category(group: .medium,
          toDos: [ToDo(title: "Exercise", todoDescription: "Walk", priority: .medium, isCompleted: true)
                 ]),
        Category(group: .low,
          toDos: [ToDo(title: "House chore", todoDescription: "Do the laundry", priority: .low, isCompleted: true),
                  ToDo(title: "Grooming", todoDescription: "Get a haircut", priority: .low, isCompleted: true)
                 ])
  ] {
    didSet {
  ...
  @objc func deleteItem() {
    ...
    reloadNCBarButtonItems(isListEmpty: toDoListIsEmpty)
    if toDoListIsEmpty { setEditing(false, animated: true) }
  }
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if !tableView.isEditing {
      ...
      print(toDoList[indexPath.section].toDos[indexPath.row].priority.rawValue.trimmingCharacters(in: CharacterSet(charactersIn: " Priority")).uppercased(), toDoList[indexPath.section].toDos[indexPath.row].title, toDoList[indexPath.section].toDos[indexPath.row].todoDescription!,
            toDoList[indexPath.section].toDos[indexPath.row].isCompleted)
